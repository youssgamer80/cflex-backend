...
import projet.cflex.oda_cflex_smart_city1.Repository.TrackerCouloirGetRepository;
import projet.cflex.oda_cflex_smart_city1.Repository.TrackerCouloirRepository;
...
public class TrackerCouloirController {
  ...
  public double distance(double lat1, double lat2, double lon1, double lon2, double el1, double el2) {
    ...
    double distance = geoCalc.calculateGeodeticCurve(reference, tracker, pointArret).getEllipsoidalDistance();
    return distance;
  }

  @GetMapping(value = "/gettrackercouloir/{longitude}/{latitude}")
  public List<TrackerCouloirGet> getTrackerCouloir(@PathVariable Double longitude, @PathVariable Double latitude) {
    double el1 = 0;
    double el2 = 0;
    ArrayList<TrackerCouloirGet> trackercouloirgetData = trackercouloirgetRepository.findAll();
    ...
    for (int i = 0; i < trackercouloirgetData.size(); i++) {
      ...
      List<TrackerCouloirGet> trackerCouloirs = trackercouloirgetRepository.findAll();
      trackerCouloirs.forEach(
          tracker -> {
            if (distance(latitude, tracker.getLatitude(), longitude, tracker.getLongitude(), el1, el2) <= 3000) {
              System.out
                  .println(distance(latitude, tracker.getLatitude(), longitude, tracker.getLongitude(), el1, el2));
              System.out.println(tracker.getImmatriculation() + " " + tracker.gettypetransport());
  ...
  public TrackerCouloirGet createTrackerCouloir(@PathVariable Double longitude, @PathVariable Double latitude) throws IOException {

    OkHttpClient client = new OkHttpClient();
    Request request = new Request.Builder()
        .url("https://api.thingspeak.com/channels/1786759/feeds.json?api_key=FBINCPKCBHKLG9MG&results=2").build();
    ...
    Request nextrequest = new Request.Builder()
        .url("https://api.thingspeak.com/channels/1757494/feeds.json?api_key=EPV5WIRQ3UOYA1RE&results=2").build();
    Response nextresponse = nextclient.newCall(nextrequest).execute();
    final String nextdata = nextresponse.body().string();
    final ResponseTracker nextres = gson.fromJson(nextdata, ResponseTracker.class);
    ...
    System.out.println("typetransport:" + TypeTransport);
      ...
      TrackerCouloir tc = new TrackerCouloir(id_couloir, nb_person, idtracker, immatriculation, lat,
    ...
    trackercouloirRepository.save(tc);
      ...
      for (int i = 0; i < trackercouloirgetData.size(); i++) {
        ...
        trackerCouloirs.forEach(
            tracker -> {
              if (distance(latitude, tracker.getLatitude(), longitude, tracker.getLongitude(), el1, el2) <=7000) {
                System.out
                    .println(distance(latitude, tracker.getLatitude(), longitude, tracker.getLongitude(), el1, el2));
                System.out.println(tracker.getImmatriculation() + " " + tracker.gettypetransport());
                trackercouloirResultats.add(tracker);
